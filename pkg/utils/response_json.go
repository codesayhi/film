...
//
// Tham số:
//   - c: *gin.Context - Context của Gin.
//   - data: interface{} - Dữ liệu muốn trả về.
//   - code: int - Mã trạng thái HTTP.
//   - message: string - Thông báo tùy chỉnh.
func SuccessJson(c *gin.Context, data interface{}, code int, message string) {
	resp := ResponseJson{
		Code:    code,
		Data:    data,
		Message: message,
...
func FailJson(c *gin.Context, code int, message string) {
	...
	// Dùng c.AbortWithStatusJSON để đảm bảo Gin ngừng các middleware phía sau (nếu có)
...
//   - code: int - Mã lỗi HTTP.
//   - message: string - Thông báo lỗi chung.
//   - details: interface{} - Chi tiết lỗi.
func FailJsonWithDetailErrors(c *gin.Context, code int, message string, details interface{}) {
	resp := ResponseJson{
		Code:    code,
		Data:    details,
		Message: message,
...
//-----------------------------------------------------------------------------------------------------------------------
// CÁC HÀM MỞ RỘNG - Gọi theo Status Code (Mapping logic giống version net/http cũ)

// ResponseOK gửi phản hồi HTTP 200 (Success).
func ResponseOK(c *gin.Context, data interface{}) {
	SuccessJson(c, data, http.StatusOK, "Thành công")
}

// ResponseCreated gửi phản hồi HTTP 201 (Created).
func ResponseCreated(c *gin.Context, data interface{}) {
	SuccessJson(c, data, http.StatusCreated, "Tạo mới thành công")
}

// ResponseDeleted gửi phản hồi HTTP 200 cho thao tác xóa.
func ResponseDeleted(c *gin.Context) {
	SuccessJson(c, nil, http.StatusOK, "Xóa thành công")
}

// ResponseBadRequest gửi phản hồi HTTP 400 (Bad Request).
func ResponseBadRequest(c *gin.Context, message string) {
	FailJson(c, http.StatusBadRequest, message)
}

// ResponseValidationError gửi phản hồi HTTP 400 kèm chi tiết lỗi Validate.
func ResponseValidationError(c *gin.Context, errors interface{}) {
	FailJsonWithDetailErrors(c, http.StatusBadRequest, "Dữ liệu không hợp lệ", errors)
}

// ResponseUnauthorized gửi phản hồi HTTP 401 (Unauthorized).
func ResponseUnauthorized(c *gin.Context) {
	FailJson(c, http.StatusUnauthorized, "Vui lòng đăng nhập")
}

// ResponseForbidden gửi phản hồi HTTP 403 (Forbidden).
func ResponseForbidden(c *gin.Context) {
	FailJson(c, http.StatusForbidden, "Bạn không có quyền truy cập")
}

// ResponseNotFound gửi phản hồi HTTP 404 (Not Found).
func ResponseNotFound(c *gin.Context, message string) {
	FailJson(c, http.StatusNotFound, message)
}

// ResponseServerError gửi phản hồi HTTP 500 (Internal Server Error).
func ResponseServerError(c *gin.Context) {
	FailJson(c, http.StatusInternalServerError, "Lỗi hệ thống nội bộ")
}
